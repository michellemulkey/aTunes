...
public class LyricsService {
	...
	private static String baseURL = "http://www.lyrc.com.ar/en/tema1en.php?artist=" + ARTIST_WILDCARD + "&songname=" + SONG_WILDCARD;
	private static String suggestionsURL = "http://www.lyrc.com.ar/en/";
	
	private static String getLyrics(Proxy proxy, String urlString) {
		try {
			URL url = new URL(urlString);
			// read html return
			String html = NetworkUtils.readURL(NetworkUtils.getConnection(url, proxy), "ISO-8859-1");
	...
	public static String getLyrics(Proxy proxy, String artist, String song) {
		String urlString = baseURL.replace(ARTIST_WILDCARD, NetworkUtils.encodeString(artist)).replace(SONG_WILDCARD, NetworkUtils.encodeString(song));
		return getLyrics(proxy, urlString);
	}
}
